...
def DefernderInField(pos, a):
    if a==0 and int(pos[4:6]) in [1,5,9,13]:
        return False
    elif a==1 and int(pos[4:6]) in [4,8,12,16]:
        return False
    elif a==2 and int(pos[4:6]) in [1,2,3,4]:
        return False
    elif a==3 and int(pos[4:6]) in [13,14,15,16]:
        return False
    else:
        return True
...
def check_tackling(pos, new_pos)-> bool:
    ...
    if new_pos[6]=="1":
        ...
        if new_pos[2:4] == new_pos[4:6]:
            return True
        elif pos_P2_i == pos_R_f and pos_P2_f==pos_R_i:
            return True
    return False
